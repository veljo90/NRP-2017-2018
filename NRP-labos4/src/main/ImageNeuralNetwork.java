...
public class ImageNeuralNetwork {
	...
	private int nbrOfIterations;
	private double error;
	private double trainingTime;
	
	private ImageDivider divider = new ImageDivider(Main.ROWS, Main.COLUMNS);

	public int getNbrOfIterations(){
		return this.nbrOfIterations;
	}
	...
	private void processTrain() throws IOException {
		...
		trainingTime = ((double) endTime - startTime) / 1000;
		nbrOfIterations = train.getIteration();
	...
	public void processWhatIs() throws IOException {
		...
		String outputFolderName = "CutPictures/" + image.getName().replace(".jpg", "");
		File outputFolder = new File(outputFolderName);
		divider.divide(image, outputFolder);
		
		String solution = "What is: " + filename + ", it seems to be: ";
	    ...
	    for (int i = 0; i < listOfFiles.length; i++) {
	    	if (listOfFiles[i].isFile()) {
	    		...
	    		solution += this.neuron2identity.get(winner) + ", ";
	      }
	    }
